...
body = html.Div([
    dbc.Row([
        ...
        justify="center"), 
    dbc.Row([
        dbc.Col(html.Div(html.H4("Last updated: %s" % datetime.datetime.strptime(last_day, '%Y-%m-%d').date())), lg=4, md=12, sm=12,  xs=12, className="last_update")
        ],
        justify="center"),
    dbc.Row([
        dbc.Col(html.Div(dbc.Alert(html.H1(children="Confirmed: %s" % format(dff.loc["confirmed"], ",")), color="primary")), lg=4, md=4, sm=12,  xs=12, className="summed-figs"), 
        dbc.Col(html.Div(dbc.Alert(html.H1(children="Deaths: %s" % format(dff.loc["deaths"], ",")), color="primary")), lg=4, md=4, sm=12, xs=12, className="summed-figs"),
        dbc.Col(html.Div(dbc.Alert(html.H1(children="Recovered: %s" % format(dff.loc["recovered"], ",")), color="primary")), lg=4, md=4, sm=12, xs=12, className="summed-figs")
        ],
        justify="center"),
    dbc.Row([
        dbc.Col(html.Div(
        ...
        dbc.Row([dbc.Col(html.Div(
            ...
            children=[
        dbc.Col(html.Div(dbc.Alert(html.H3(children="Covid-19 World Map"), color="light")), lg=6, md=6, sm=12, xs=12, className="graph-title"),
        dcc.Graph(
        ...
        dbc.Col(html.Div(
            className="graph-wrapper", 
            children=[
        dbc.Col(html.Div(dbc.Alert(html.H3(children="Compared Daily Growth"), color="light")), lg=6, md=6, sm=12, xs=12, className="graph-title"),
        dcc.Graph(
...
def create_datasets(input_case, input_countries):
    ...
    for country in input_countries:
        last_d_filtered_df = df.copy()[df["date"] == last_day]
...
#         print(datasets)     
    ...
    second_layout = go.Layout(
        ...
        xaxis_title="Days since first occurence",
        margin=dict(
            l=0,
            r=0,
            b=0,
